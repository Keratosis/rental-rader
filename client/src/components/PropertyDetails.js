...
function PropertyDetails({ match }) {
  ...
  const propertyId = match.params.id; // Extracting the listing ID from the URL params
  ...
  useEffect(() => {
    ...
    fetch(`/similarProperties/${propertyId}`) 
      .then(res => res.json())
      .then(data => {
        ...
        {similarProperties.length > 0 ? (
          similarProperties.map(property => (
            <div key={property.id}>
              ...
              <p>{property.location_details}</p>
              <p>Rent: {property.rent}</p>
              <p>Bedrooms: {property.bedrooms}</p>
              <p>Bathrooms: {property.bathrooms}</p>
              <p>Description: {property.description}</p>
              <p>Landlord Name: {property.landlord_name}</p>
            </div>
          ))
        ) : (
